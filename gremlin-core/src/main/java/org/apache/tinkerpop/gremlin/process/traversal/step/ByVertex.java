 ...
 * @author zhf
 */
public abstract class ByVertex<S, E> extends AbstractStep<S, E> implements Barrier<E>, Generating<E, E> {
    protected Supplier<E> seedSupplier;
    protected BinaryOperator<E> reducingBiOperator;
    private boolean hasProcessedOnce = false;
    private E seed = null;

    public ByVertex(final Traversal.Admin traversal) {
        super(traversal);
    }
...
//        final Traverser.Admin<E> traverser = this.getTraversal().getTraverserGenerator().generate(this.generateFinalResult(this.seed), (Step<E, E>) this, 1L);
        final Traverser.Admin<E> traverser = this.getTraversal().getTraverserGenerator().generate(this.seed, (Step<E, E>) this, 1L);
        this.seed = null;
        return traverser;
    }

    @Override
    public ByVertex<S, E> clone() {
        final ByVertex<S, E> clone = (ByVertex<S, E>) super.clone();
